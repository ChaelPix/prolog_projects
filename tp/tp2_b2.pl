...
entrees( ['Foie gras', 'Salade gourmande', 'Crudités', 'Tomates mozzarella', 'Terrines de foie de volaille'] ).
viande( ['entrecote', 'magret_canard', 'dinde_fermiere']).
poisson( ['truite_meuniere', 'brochet_de_loire', 'cube_de_bar']).
dessert( ['mousse_chocolat', 'sorbet', 'ile_flottante', 'poire_belle_helene']).

% regarder si element est dans la liste
element-de( X, [X | _] ).
element-de( X, [_ | Y] ) :- element-de( X, Y ).

% regarde si viande ou poisson est dans la luste
plat(X) :- 
    (viande(L) ; poisson(L)),
    element-de(X, L), !.
